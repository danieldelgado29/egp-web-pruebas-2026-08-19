...
(() => {
  ...
  async function loadData(){
    ...
    if(
      new URL(location.href)
        .searchParams
        .get('copiar_letras_elena')==='1'
    ){
      setTimeout(
        ()=>void egpMigrarLetrasPublicasAElena(),
        350
      );
    }
  }
  ...
  /*
   * EGP — MIGRACION UNICA:
   * letra pública -> Cancionero Elena visual.
   *
   * Solo se ejecuta con:
   * ?copiar_letras_elena=1
   *
   * Nunca reemplaza cajas manuales ni dibujos existentes.
   */
  async function egpMigrarLetrasPublicasAElena(){
    if(window.__egpMigrandoLetrasElena)return;
    window.__egpMigrandoLetrasElena=true;
    ...
    status.textContent='Preparando Cancionero Elena…';

    document.body.append(status);

    if(!navigator.onLine){
      status.textContent=
        'Sin Internet. No se inició la migración.';
      window.__egpMigrandoLetrasElena=false;
      return;
    }

    const songs=
      state.songs.filter(song=>
        String(song?.cancioneroElena||'').trim()
      );
    ...
    for(let i=0;i<songs.length;i++){
      ...
      status.textContent=
        `Cancionero Elena ${i+1}/${songs.length} · ${song.titulo}`;
      ...
      try{
        ...
        const existing=
          await loadRemoteImageEdit(
            song.id,
            'elena',
            'songbook'
          ) ||
          await offlineStoreGet(
            'imageEdits',
            editId
          ) ||
          null;
        ...
        const boxes=
          Array.isArray(existing?.textBoxes)
            ? existing.textBoxes
            : [];
        ...
        const importedId=
          importedElenaBoxId(song.id);

        const importedIndex=
          boxes.findIndex(box=>
            box?.id===importedId ||
            box?.importSource==='cancioneroElena'
          );

        const manualText=
          boxes.some((box,index)=>{
            if(index===importedIndex)return false;
         ...
         * si Elena ya hizo cajas o dibujos manuales,
         * esta migración NO agrega ni reemplaza nada.
         */
        if(manualText || manualDrawing){
          protegidas++;
          console.log(
            '[EGP ELENA] PROTEGIDA:',
            song.id,
        ...
        const result=
          await syncElenaSongTextToImageEdit(
            song,
            ''
          );
    ...
    const resumen=
      `LISTO · nuevas ${sincronizadas}`+
      ` · ya importadas ${yaImportadas}`+
    ...
    /*
     * Quitar el parámetro para que una recarga
     * normal no vuelva a iniciar la migración.
     */
    try{
