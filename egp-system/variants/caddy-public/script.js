...
async function iniciarFirebase(firebaseConfig) {
  ...
  try {
    ...
    const procesarEstadoRemoto = async (snapshot) => {
      ...
      const lanAutoritativa =
        estado.egpLanConfig?.__egp_core_v1 === true &&
        estado.egpLanConfig?.show_active === true;
      ...
      if (!lanAutoritativa) {
        estado.idsRepertorioRemoto =
          idsRemotos ? new Set(idsRemotos.map(String)) : null;
      }

      const listaAplicable = lanAutoritativa
        ? String(estado.egpLanConfig?.lista_activa || estado.modo || "principal-diario")
        : ((estado.idsRepertorioRemoto || modoValido(listaRemota))
            ? listaRemota
            : "principal-diario");
      ...
      if (!lanAutoritativa) {
        ...
        estado.configRemota.pedidos_panel_lista =
          Array.isArray(datos.pedidos_panel_lista)
            ? datos.pedidos_panel_lista
            : [];
      }

      actualizarEstadoFirebase(
        desdeCache ? "Copia local" : "En línea",
        desdeCache ? "" : "online"
      );
      ...
      if (!lanAutoritativa) {
        ...
        if (
          !estado.modoForzado &&
          (estado.modo !== listaAplicable || bibliotecaCambio)
        ) {
...
const EGP_CORE_PUBLIC_URL = EGP_AUDIT_LOCAL
  ? "http://10.10.10.2:8796"
  : (location.hostname === "elenagirjoaba.com"
      ? location.origin + "/__egp_core"
      : "https://core.elenagirjoaba.com");
let egpCoreRepertoireSignatureV1 = "";
let egpCoreUiSignatureV1 = "";

function egpPedidosPanelActivoV85() {
  if (estado.egpLanConfig?.ok) {
    return estado.egpLanConfig.show_active === true &&
           estado.egpLanConfig.pedidos_panel === true;
  }
...
function egpPedidosModoActualV4() {
  if (estado.egpLanConfig?.ok) {
    return estado.egpLanConfig.pedidos_modo === "uno_por_turno"
      ? "uno_por_turno"
      : "libre";
  }

  return estado.configRemota.pedidos_modo === "uno_por_turno"
    ? "uno_por_turno"
    : "libre";
}
...
async function egpFetchCorePublicV1(path, options = {}, timeoutMs = 1200) {
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), timeoutMs);
  try {
    return await fetch(`${EGP_CORE_PUBLIC_URL}${path}`, {
      cache: "no-store",
      ...options,
      signal: controller.signal
    });
...
async function egpSincronizarLanV85(forzar = false) {
  ...
  try {
    const stateResponse = await egpFetchCorePublicV1("/api/state", {}, 1200);
    if (!stateResponse.ok) {
      throw new Error(`Local Core HTTP ${stateResponse.status}`);
    }

    localState = await stateResponse.json();

    if (localState?.ok !== true) {
      throw new Error("Local Core inválido");
    }

    const pc = localState.publicConfig;
    ...
    if (
      ...
      throw new Error("Local Core todavía no tiene publicConfig");
    }
    ...
    const listaLan = String(
      config.lista_activa ||
      estado.modo ||
      "principal-diario"
    );

    const idsLan = Array.isArray(config.repertorio_activo_ids)
      ? config.repertorio_activo_ids.map(String)
      : null;

    estado.idsRepertorioRemoto = idsLan
      ? new Set(idsLan)
      : null;

    const listaAplicableLan =
      (estado.idsRepertorioRemoto instanceof Set || modoValido(listaLan))
        ? listaLan
        : "principal-diario";

    estado.configRemota.pedidos_panel =
      config.show_active === true &&
      config.pedidos_panel === true;

    estado.configRemota.pedidos_modo =
      config.pedidos_modo === "uno_por_turno"
        ? "uno_por_turno"
        : "libre";
    ...
    estado.configRemota.mostrar_cola =
      config.mostrar_cola !== false;

    estado.configRemota.lista_activa = listaAplicableLan;
    estado.configRemota.listaActiva = listaAplicableLan;

    estado.configRemota.repertorio_nombre =
      String(config.repertorio_nombre || "");

    estado.configRemota.repertorio_activo_ids =
      idsLan || [];

    estado.configRemota.repertorioActivoIds =
      idsLan || [];

    estado.configRemota.lugar =
      String(config.lugar || localState?.show?.venue || "");

    estado.configRemota.perfil_clientes =
      ["alto","medio","bajo"].includes(config.perfil_clientes)
        ? config.perfil_clientes
        : "medio";

    estado.configRemota.uso_publicidad =
      config.uso_publicidad === true;

    if (config.show_active === true && config.show_id) {
      estado.configRemota.inicio_show =
        Number(config.show_id) ||
        estado.configRemota.inicio_show;
      ...
      estado.configRemota.show_activo =
        localState?.show?.active === true;
    }

    const rows = Array.isArray(localState.queue)
      ? [...localState.queue].sort(
          (a,b)=>
            (Number(a.position)||0) -
            (Number(b.position)||0)
        )
    ...
    estado.configRemota.cola = rows
      .map(x=>String(x.id||""))
      .filter(Boolean);

    estado.configRemota.tocadas = rows
      .filter(x=>x.played===true)
      .map(x=>String(x.id||""))
      .filter(Boolean);

    const firmaRepertorioCoreV1 = JSON.stringify([
      listaAplicableLan,
      idsLan || []
    ]);

    const repertorioCoreListo =
      Array.isArray(estado.todas) &&
      estado.todas.length > 0;

    const repertorioCoreCambio =
      firmaRepertorioCoreV1 !== egpCoreRepertoireSignatureV1;

    if (
      repertorioCoreListo &&
      !estado.modoForzado &&
      (
        estado.modo !== listaAplicableLan ||
        repertorioCoreCambio
      )
    ) {
      aplicarModo(listaAplicableLan,false);
    }

    if (repertorioCoreListo) {
      egpCoreRepertoireSignatureV1 = firmaRepertorioCoreV1;
    }

    // El Core se consulta cada 2.5 s, pero la interfaz solo debe volver a
    // dibujarse si realmente cambió algo visible. Esto evita el parpadeo
    // periódico de la página LAN.
    const firmaUiCoreV1 = JSON.stringify({
      show_active: config.show_active === true,
      show_id: String(config.show_id || ""),
      lista_activa: listaAplicableLan,
      repertorio_nombre: String(config.repertorio_nombre || ""),
      repertorio_activo_ids: idsLan || [],
      pedidos_panel: config.pedidos_panel === true,
      pedidos_modo: config.pedidos_modo === "uno_por_turno" ? "uno_por_turno" : "libre",
      pedidos_whatsapp: config.pedidos_whatsapp !== false,
      mostrar_cola: config.mostrar_cola !== false,
      lugar: String(config.lugar || localState?.show?.venue || ""),
      perfil_clientes: String(config.perfil_clientes || "medio"),
      uso_publicidad: config.uso_publicidad === true,
      cola: estado.configRemota.cola || [],
      tocadas: estado.configRemota.tocadas || []
    });

    if (firmaUiCoreV1 !== egpCoreUiSignatureV1) {
      egpCoreUiSignatureV1 = firmaUiCoreV1;
      sincronizarInterfazRemota();
    }

  } catch (error) {
    if (estado.egpLanConfig?.__egp_core_v1 === true) {
      estado.egpLanConfig = null;
    }
    return;
  }
  ...
  try {
    if (
      ...
      if (ordersResponse.ok) {
        ...
        estado.egpPedidosLan =
          data?.ok && Array.isArray(data.orders)
            ? data.orders
            : [];
      }
    } else {
    ...
    // 8790 no afecta al estado recibido de 8788.
  }
  ...
  const firmaColaDespues = JSON.stringify({
    cola: (estado.configRemota.cola || []).map(String),
    tocadas: (estado.configRemota.tocadas || []).map(String)
  });
...
/* EGP LAN — el arranque real se hace en iniciar(), después de cargarDatos(). */
...
async function iniciar() {
  capturarDOM();
  // 8788 se inicia después de cargarDatos(): DOM + canciones deben existir.
  ...
  try {
    ...
    // EGP 8788: primera lectura solo cuando el catálogo y el DOM ya están listos.
    await egpSincronizarLanV85(true);
    egpProgramarLanV85(2500);
